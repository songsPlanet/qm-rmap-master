...
class AnimationRoute {
  ...
  declare pauseIndex: number;

  declare pauseStatus: boolean;

  declare interval: number;

  constructor(map: Map, route: any) {
    this.map = map;
    this.route = route;
  ...
  loop(idx: number) {
    let newIndex = this.pauseStatus === false ? idx : this.pauseIndex;
    this.task(newIndex);
  ...
  task(index: any) {
    const coords = this.route.features[0].geometry.coordinates;
    ...
    if (this.map) {
      (this.map.getSource('line-animate') as GeoJSONSource).setData(geojson);
      (this.map.getSource('point') as GeoJSONSource).setData(pointGeojson);
    }
  }
  animateLine() {
    let idx = 0;
